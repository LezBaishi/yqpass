  ...
  <el-row class="footer">
	...
	<el-row class="transfer_record" v-if="ins_seen">
		<el-collapse v-model="activeNames" @change="handleChange" accordion>
			<el-collapse-item v-for="(value, key) in flowLogs" :name="key">
				<template slot="title">
					<div>工单状态：<b>{{ value.state }}</b></div>
				</template>
				<div>工单状态：<b>{{value.state}}</b>，处理人：<b>{{value.participant}}</b>，处理结果：<b>{{value.transition}}</b>，处理意见：<b>{{value.suggestion}}</b>， 处理时间：{{value.gmt_modified}}</div>
			</el-collapse-item>
		</el-collapse>
	</el-row>  
...
<script>
import { postTicketList,getWorkflowInitState,getTicketDetail,getTicketFlowlogs } from '../api/api'
export default {
  data () {
    return {
	...
	handleChange(val) {
  	...
  	readIns () {
  		if (this.$data.ins_seen == false){
			...
			getTicketFlowlogs(this.$route.query.id).then((response)=> {
				console.log(response)
				if (response.data.code == 1) {
