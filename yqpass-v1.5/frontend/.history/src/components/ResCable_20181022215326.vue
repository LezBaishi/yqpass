  	...
  	<el-row class="container">
      		...
      		<el-form ref="search_form" :model="search_form" label-width="80px">
       		<el-row>
      			...
      			<el-col :offset="3" :span="8">
					<el-form-item label="光缆信息">
						<el-col :span="11">
							<el-form-item prop="building">
								<el-input placeholder="请输入楼栋" v-model="search_form.building" style="width:100%;"></el-input>
							</el-form-item>
						</el-col>
						<el-col class="line" :span="2">-</el-col>
						<el-col :span="11">
							<el-form-item prop="room">
								<el-input placeholder="请输入机房号" v-model="search_form.room" style="width:100%;"></el-input>
							</el-form-item>
						</el-col>
					</el-form-item>
				</el-col>
        ...
        <el-row>
        	<el-col>
				...
				<el-button style='float: right; margin-right:10px;' type="primary" size="mini" round @click="exportInfo">导出</el-button>
				<el-button style='float: right;' type="primary" size="mini" round @click="setPage">设置</el-button>
				<el-button style='float: right;' type="primary" size="mini" round @click="newDialogForm('dialog_form')">新增</el-button>
			</el-col>
		</el-row>
		<el-row>
...
import { getCableList,getBuildingRoomList,newCableList,editCableList,delCableList } from '../api/api'
export default {
  data () {
    return {
	  tableData: [],
  ...
  methods: {
	...
	delAll () {
		...
		for (var i=0; i<delList.length; i++) {
			idList += i === delList.length-1? delList[i].id:delList[i].id+",";
		};
		console.log(idList);
		let paramsData = {};
		paramsData["delete_str"] = idList.toString();
		var parmas = JSON.stringify(paramsData);
		console.log(rowValue);
    	this.$confirm('是否确定删除该部分数据', '提示', {
    		confirmButtonText: '确定',
    		cancelButtonText: '取消',
    		type: 'warning'
    	}).then(() => {
			delCableList(parmas).then((response)=> {
				console.log(response);
				if (response.data.code == 1) {
					this.getocableList();
					this.$notify({
	...
	newDialogForm(formName) {
		...
		this.dialog_form = this.dialog_form_empty;
	},
	submitDialogForm(formName) {
		let paramsData = {};
		paramsData["building_A"] = this.dialog_form.building_A;
		paramsData["room_A"] = this.dialog_form.room_A;
		paramsData["name_A"] = this.dialog_form.name_A;
		paramsData["building_Z"] = this.dialog_form.building_Z;
		paramsData["room_Z"] = this.dialog_form.room_Z;
		paramsData["name_Z"] = this.dialog_form.name_Z;
		paramsData["core_num"] = this.dialog_form.core_num;
		paramsData["ocable_length"] = this.dialog_form.ocable_length;
		paramsData["ocable_type"] = this.dialog_form.ocable_type;
		paramsData["ocable_level"] = this.dialog_form.ocable_level;
		paramsData["ofiber_type"] = this.dialog_form.ofiber_type;
		paramsData["notes"] = this.dialog_form.notes;
		// console.log(paramsData);
		var parmas = JSON.stringify(paramsData);
		if (this.dialog_form == this.dialog_form_empty) { 
			newCableList(parmas).then((response)=> {
				console.log(response);
				if (response.data.code == 1) {
			...
			editCableList(id, parmas).then((response)=> {
				console.log(response);
				if (response.data.code == 1) {
					this.dialogFormVisible = false;
					this.getocableList();
    ...
    //   	edit: true
  	...
  	deleteRow (index, rows) {
		...
		let paramsData = {};
		paramsData["delete_str"] = rowValue.id.toString();
		var parmas = JSON.stringify(paramsData);
		console.log(rowValue);
