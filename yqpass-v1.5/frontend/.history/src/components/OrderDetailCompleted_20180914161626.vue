...
<script>
import { postTicketList,getWorkflowInitState,getTicketDetail } from '../api/api'
export default {
  data () {
    return {
	...
	getOrderDetail (id) {
		getTicketDetail(id).then((response)=> {
			...
			if (response.data.code == 1) {
				...
				let list = [{}];
				// list.push({title: response.data.data.title})
				for (var i=0; i<tableList.length; i++) {
					list[tableList[i].field_key] = tableList[i].field_value;
				};
				console.log(list);
				this.orderMain_form = list;
				this.orderMain_form.title = title: response.data.data.title
			}else{
				return response[msg]
			}
		})
