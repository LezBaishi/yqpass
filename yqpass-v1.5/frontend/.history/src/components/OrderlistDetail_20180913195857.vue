<template lang="html"> 
  ...
  <el-row class="footer">
  	<el-row>
		...
		<el-button type="primary" icon="el-icon-success" v-for="(value, key) in button_list" :key="key" @click="handleData(value.transtion_id)">{{ value.transtion_name }}</el-button>
  	</el-row>
  ...
  methods: {
	...
	handleData (key) {
		...
		if (key == 2) {
			...
			json["transition_id"] = key;
			json["title"] = that.orderMain_form.project_name;
			json["application_date"] = that.applyDate;
			json["open_date"] = that.orderMain_form.date;
			json["person1"] = that.orderMain_form.CMer_name;
			json["phone1"] = that.orderMain_form.CMer_phone;
			json["email1"] = that.orderMain_form.CMer_email;
