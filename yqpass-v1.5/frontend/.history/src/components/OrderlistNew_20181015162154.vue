	...
	uniqueArray (array) {
  	...
  	get_machineBuilding: function (building) {
		...
		// console.log(list.length);		
  		let temRoom = [];
  		for (var i=0; i<110; i++){
			//   console.log(this.allRoom[i].building)
  			if (building == JSON.parse(JSON.stringify(this.allRoom[i].building))){
  				temRoom.push({label: JSON.parse(JSON.stringify(this.allRoom[i].label)), value: JSON.parse(JSON.stringify(this.allRoom[i].label))})
  			}
  		}
  		console.log(temRoom)
