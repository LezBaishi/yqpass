		...
		getUserList({params : paramsData}).then((response)=> {
			...
			if (response.data.code == 1)	{
				...
				for (var i=0; i<list.length; i++) {
					...
					this.tableData[i].active==true?this.tableData.active=='激活':this.tableData.active=='未激活';
					this.tableData[i].admin==true?this.tableData.admin=='管理员':this.tableData.admin=='普通用户';
				};
