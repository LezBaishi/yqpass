					...
					<el-row>
						<el-col :span="8">
							<el-form-item label="A端业务设备楼栋" :label-width="formLabelWidth">							
								<el-select v-model="data_dialogForm.building_A" placeholder="请选择" filterable v-on:change="get_machineBuilding($event)">
						...
						<el-col :span="8" :push="2">
							<el-form-item label="机房号" label-width="120px">
								<el-select v-model="data_dialogForm.room_A" placeholder="请选择" filterable v-on:change="get_machineRoom($event)">
					...
					<el-row>
						<el-col :span="8">
							<el-form-item label="Z端业务设备楼栋" :label-width="formLabelWidth">
								<el-select v-model="data_dialogForm.building_Z" placeholder="请选择" v-on:change="get_machineBuilding($event)">
									<el-option v-for="item in machineBuilding_option" :key="item.value" :label="item.label" :value="item.value"></el-option>
								</el-select>
							</el-form-item>
						</el-col>
						<el-col :span="8" :push="2">
							<el-form-item label="机房号" label-width="120px">
								<el-select v-model="data_dialogForm.room_Z" placeholder="请选择" v-on:change="get_machineRoom($event)">
  		...
  		<el-table :data="order_form" ref="table" tooltip-effect="dark" border stripe style="width:100%">
  			...
  			<el-table-column label="A端业务设备位置" align="center">
  				<el-table-column label="机楼" width="110" align="center">
  					<template slot-scope="scope">
  						<el-select v-show="scope.row.edit" v-model="scope.row.building_A" placeholder="请选择机楼" v-on:change="get_machineBuilding($event)">
  							<el-option v-for="item in machineBuilding_option" :key="item.value" :label="item.label" :value="item.value"></el-option>
  						</el-select>
			...
			<el-table-column label="Z端业务设备位置" align="center">
				<el-table-column label="机楼" width="110" align="center">
					<template slot-scope="scope">
						<el-select v-show="scope.row.edit" v-model="scope.row.building_Z" placeholder="请选择机房" v-on:change="get_machineBuilding($event)">
							<el-option v-for="item in machineBuilding_option" :key="item.value" :label="item.label" :value="item.value"></el-option>
						</el-select>
...
export default {
  ...
  methods: {
	...
	get_machineRoom:function (room) {
		console.log(room);
		// this.room_A_text = this.get_roomByid(room);
		// console.log(this.room_A_text);
	},
	...
	submitDialogForm(formName) {
		...
		if (this.data_dialogForm == this.order_form_empty) {
			...
			// this.get_roomByid(this.order_form_empty.building_A, this.order_form_empty.room_A, function(roomid){
			// 	console.log(roomid);
			// 	var that = this;
			// 	// that.order_form_empty.fk_rid_A = roomid;
			// 	that.room_A_id = roomid;
			// 	console.log(that.room_A_id);
			// 	// console.log(this.order_form_empty.fk_rid_A);
			// 	// this.room_A_id = roomid;
			// 	this.get_roomByid(this.order_form_empty.building_Z, this.order_form_empty.room_Z, function(roomid){
			// 		var that = this;
			// 		that.order_form_empty.fk_rid_Z = roomid;
			// 		// this.room_Z_id = roomid;
			// 	});
			// });
			console.log(this.room_Z_id);
			this.order_form.push({
				...
				building_A:this.order_form_empty.building_A,
				room_A:this.order_form_empty.room_A,
				// fk_rid_A:this.room_A_id,
				...
				building_Z:this.order_form_empty.building_Z,
				room_Z:this.order_form_empty.room_Z,
				// fk_rid_Z:this.room_Z_id,
