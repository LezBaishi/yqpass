  	...
  	<el-row class="header">
  		...
  		<el-row style="margin-bottom:15px;">
  			<el-col :span="12" text-align="left">
  				<p v-model="orderNum" style="font-size:14px;float: left;font-weight:bold;">申请施工单号：{{ orderMain_form.orderNum }}</p>
  			</el-col>
  			<el-col :span="12" text-align="right">
  				<p style="font-size:14px;float: right;font-weight:bold;">申请时间：{{ orderMain_form.application_open_date }}</p>
  			</el-col>
  		</el-row>
  		<el-row>
  			<el-col :span="11">  			
  			...
  			<el-col :span="11" :push="2">  			
	  			<el-form-item label="开通时间">
	  			  <el-open_date-picker type="open_date" placeholder="选择开通日期" value-format="yyyy-MM-dd" v-model="orderMain_form.open_date" style="width:100%;"></el-open_date-picker>	
	  			</el-form-item>
  			</el-col>
  		</el-row>
  		<el-row>
...
<script>
import { postTicketList,getWorkflowInitState,geTicketDetail } from '../api/api'
export default {
  data () {
    return {
      applyopen_date: new open_date().getTime(),
      orderMain_form: [{
		orderNum: '',
      	project_name: '',
      	open_date: '',
  ...
  methods: {
	...
	getopen_date (value) {
		let open_date = new open_date(value);
        let y = open_date.getFullYear();
        let MM = open_date.getMonth() + 1;
        MM = MM < 10 ? ('0' + MM) : MM;
        let d = open_date.getopen_date();
        d = d < 10 ? ('0' + d) : d;
        let h = open_date.getHours();
        h = h < 10 ? ('0' + h) : h;
        let m = open_date.getMinutes();
        m = m < 10 ? ('0' + m) : m;
        let s = open_date.getSeconds();
        s = s < 10 ? ('0' + s) : s;
        //return y + '-' + MM + '-' + d + ' ' + h + ':' + m + ':' + s;
        return y + '-' + MM + '-' + d
	},
	getOrderDetail (id) {
		geTicketDetail(id).then((response)=> {
			console.log(response);
			if (response.data.code == 1) {
				this.button_list = response.data.data.transition_list;
	...
	handleData (key) {
		...
		if (key == 1) {
			var that = this;
			let open_date = that.getopen_date(that.applyopen_date);
			let json = {};
			json["workflow_id"] = 1;
			json["transition_id"] = key;
			json["title"] = that.orderMain_form.project_name;
			json["application_open_date"] = open_date;
			json["open_open_date"] = that.orderMain_form.open_date;
		...
		if (key == 2) {
			...
			json["transition_id"] = key;
			json["title"] = that.orderMain_form.project_name;
			json["application_open_date"] = that.applyopen_date;
			json["open_open_date"] = that.orderMain_form.open_date;
			json["person1"] = that.orderMain_form.person1;
			json["phone1"] = that.orderMain_form.phone1;
			json["email1"] = that.orderMain_form.email1;
  	...
  	readIns () {
  		if (this.$data.ins_seen == false){
  			...
  			console.log(this.open_dateFormatter(this.$data.applyopen_date))
  		}else {
