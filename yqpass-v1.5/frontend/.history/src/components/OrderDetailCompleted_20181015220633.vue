	...
	getOrderDetail (id) {
		...
		getTicketDetail(id).then((response)=> {
			...
			if (response.data.code == 1) {
				...
				for (var i=0; i<list.application_detail.length; i++) {
					console.log(list.application_detail[i])
					list["field_num"] = list.application_detail[i].field_num;
					list["bus_sys_name"] = list.application_detail.bus_sys_name;
				};
				console.log(list);
