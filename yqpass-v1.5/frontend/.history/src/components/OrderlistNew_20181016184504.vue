...
export default {
  ...
  methods: {
  	get_machineBuilding: function (building) {
			...
			getBuildingRoomList({params : paramsData}).then((response)=> {
				...
				if (response.data.code = 1) {
					...
					for (var i=0; i<list.length; i++) {
						// console.log(list[i].building);
						this.allRoom.push({
							// id: JSON.parse(JSON.stringify(list[i].id)),
							// building: JSON.parse(JSON.stringify(list[i].building)),
							// label: JSON.parse(JSON.stringify(list[i].room))
							id: list[i].id,
		...
		// console.log(this.allRoom[0]);
		// let list = JSON.parse(this.allRoom);
		// console.log(list);		
  		
		// for (var i=0; i<110; i++) {

		// };
  		// for (var val of this.allRoom){
		// 	  console.log(val.building)
  		// 	if (building == val.building){
  		// 		temRoom.push({label: val.label, value: val.label})
  		// 	}
  		// }
  		console.log(temRoom)
