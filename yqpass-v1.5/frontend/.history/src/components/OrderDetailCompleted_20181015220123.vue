	...
	getOrderDetail (id) {
		...
		getTicketDetail(id).then((response)=> {
			...
			if (response.data.code == 1) {
				...
				console.log(tableList.application_detail)
				for (var i=0; i<tableList.application_detail.length; i++) {
					list["field_num"] = tableList.application_detail[i].field_num;
					list["bus_sys_name"] = tableList.application_detail.bus_sys_name;
				};
				console.log(list);
