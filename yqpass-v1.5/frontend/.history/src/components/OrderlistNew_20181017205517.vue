  ...
  <div>
    ...
    <el-row> 
  	...
  	<el-row class="header">
  		...
  		<el-row>
  			<el-col :span="11">  			
	  			<el-form-item label="项目名称" prop="project_name" label-width="80px" class="is-required">
  		...
  		<el-row>
	  		<el-col :span="7">  			
		  		<el-form-item label="移动人员" prop="CMer_name" label-width="80px" class="is-required">
		  			<el-input placeholder="请填写姓名" v-model="orderMain_form.CMer_name" clearable></el-input>
		  		</el-form-item>
	  		</el-col>
	  		<el-col :span="7" :offset="1">  			
		  		<el-form-item label="电话" prop="CMer_phone" label-width="80px" class="is-required">
		  			<el-input placeholder="请填写手机号" v-model="orderMain_form.CMer_phone" clearable></el-input>
		  		</el-form-item>
	  		</el-col>  
	  		<el-col :span="7" :offset="1">  			
		  		<el-form-item label="邮箱" prop="CMer_email" label-width="80px" class="is-required">
		...
		<el-row>			  			
	  		<el-col :span="7">  			
		  		<el-form-item label="配合人员" prop="CPer_name" label-width="80px" class="is-required">
		  			<el-input placeholder="请填写姓名" v-model="orderMain_form.CPer_name" clearable></el-input>
				</el-form-item>
	  		</el-col>
	  		<el-col :span="7" :offset="1">  			
		  		<el-form-item label="电话" prop="CPer_phone" label-width="80px" class="is-required">
		  			<el-input placeholder="请填写手机号" v-model="orderMain_form.CPer_phone" clearable></el-input>
		  		</el-form-item>
	  		</el-col>  
	  		<el-col :span="7" :offset="1">  			
		  		<el-form-item label="邮箱" prop="CPer_email" label-width="80px" class="is-required">
		...
		<el-button type="primary" style="float:left;margin-bottom:10px;" size="mini" round @click="newDialogForm('data_dialogForm')">新增表行</el-button>  
			<el-dialog title="新增申请项目" :visible.sync="dialogFormVisible" width="66%" center class="dialog">
					...
					<el-row>
						<el-col :span="8">
							<el-form-item label="A端业务设备楼栋" :label-width="formLabelWidth">							
								<el-select v-model="data_dialogForm.A_machineBuilding" placeholder="请选择" filterable v-on:change="get_machineBuilding($event)">
						...
						<el-col :span="8" :push="2">
							<el-form-item label="机房号" label-width="120px">
								<el-select v-model="data_dialogForm.A_machineRoom" placeholder="请选择" filterable v-on:change="get_machineRoom($event)">
									<el-option v-for="item in machineRoom_option" :key="item.key" :label="item.label" :value="item.value"></el-option>
								</el-select>
							</el-form-item>
						</el-col>							
					</el-row>
				...
				<div slot="footer" class="dialog-footer">					
					...
					<!-- <el-button type="primary" @click="innerVisible = true">在用电路申请</el-button> -->
...
<script>
import { postTicketList,getWorkflowInitState,getBuildingRoomList } from '../api/api'
export default {
  data () {
  ...
  methods: {
	...
	get_roomByid (building, room, cb) {
		let paramsData = {};
		// paramsData["room_id"] = roomid;
		paramsData["building"] = building;
		paramsData["room"] = room;
		// let room_id = ''
		getBuildingRoomList({params:paramsData}).then((response)=> {
			// console.log(response);
			if (response.data.code == 1){
				let room_data = response.data.data.value;
	...
	get_machineRoom:function (value) {
		console.log(value);
		// this.A_machineRoom_text = this.get_roomByid(room);
		// console.log(this.A_machineRoom_text);
	},
	...
	submitDialogForm(formName) {
		// console.log(this.order_form_empty.main_or_spare);
		...
		if (this.data_dialogForm == this.order_form_empty) {
			...
			// var main_or_spare_text = ''
			// if (this.order_form_empty.main_or_spare=="main") {
			// 	main_or_spare_text='主用'
			// }else if (this.order_form_empty.main_or_spare=="spare") {
			// 	main_or_spare_text='备用'
			// }else{
			// 	main_or_spare_text=''
			// }
			// let A_machineRoom_id = '';
			// let Z_machineRoom_id = '';
			console.log(this.order_form_empty.A_machineRoom);
			this.get_roomByid(this.order_form_empty.A_machineBuilding, this.order_form_empty.A_machineRoom, function(roomid){
				console.log(roomid);
				var that = this;
				// that.order_form_empty.fk_rid_A = roomid;
				that.A_machineRoom_id = roomid;
				console.log(that.A_machineRoom_id);
				// console.log(this.order_form_empty.fk_rid_A);
				// this.A_machineRoom_id = roomid;
				this.get_roomByid(this.order_form_empty.Z_machineBuilding, this.order_form_empty.Z_machineRoom, function(roomid){
					var that = this;
					that.order_form_empty.fk_rid_Z = roomid;
					// this.Z_machineRoom_id = roomid;
					this.order_form.push({
					// field_num:this.nextId++,
					bus_sys_name:that.order_form_empty.bus_sys_name,
					department1_A:that.order_form_empty.department1_A,
					person1_A:that.order_form_empty.person1_A,
					phone1_A:that.order_form_empty.phone1_A,
					department2_A:that.order_form_empty.department2_A,
					person2_A:that.order_form_empty.person2_A,
					phone2_A:that.order_form_empty.phone2_A,
					A_machineBuilding:that.order_form_empty.A_machineBuilding,
					A_machineRoom:that.order_form_empty.A_machineRoom,
					fk_rid_A:that.A_machineRoom_id,
					rack_num_A:that.order_form_empty.rack_num_A,
					device_name_A:that.order_form_empty.device_name_A,
					port_name_A:that.order_form_empty.port_name_A,
					port_type_A:that.order_form_empty.port_type_A,
					// cable_list:cable_data,
					department1_Z:that.order_form_empty.department1_Z,
					person1_Z:that.order_form_empty.person1_Z,
					phone1_Z:that.order_form_empty.phone1_Z,
					department2_Z:that.order_form_empty.department2_Z,
					person2_Z:that.order_form_empty.person2_Z,
					phone2_Z:that.order_form_empty.phone2_Z,
					Z_machineBuilding:that.order_form_empty.Z_machineBuilding,
					Z_machineRoom:that.order_form_empty.Z_machineRoom,
					fk_rid_Z:that.Z_machineRoom_id,
					rack_num_Z:that.order_form_empty.rack_num_Z,
					device_name_Z:that.order_form_empty.device_name_Z,
					port_name_Z:that.order_form_empty.port_name_Z,
					port_type_Z:that.order_form_empty.port_type_Z,
					bandwidth:that.order_form_empty.bandwidth,
					main_or_spare:that.order_form_empty.main_or_spare,
					// main_or_spare_view:main_or_spare_text,  
					notes: that.order_form_empty.notes,
					// edit:false
				});
			...
			console.log(this.Z_machineRoom_id);
			
		} else {
			let indexId = this.dialogFormIndexId;
			// var main_or_spare_text = ''
			// if (this.data_dialogForm.main_or_spare=="main") {
			// 	main_or_spare_text='主用'
			// }else if (this.data_dialogForm.main_or_spare=="spare") {
			// 	main_or_spare_text='备用'
			// }else{
			// 	main_or_spare_text=''
			// }
			// this.data_dialogForm.main_or_spare_view = main_or_spare_text;
			console.log(this.data_dialogForm.fk_rid_A);
	...
	editDialogForm (index, rows) {
		...
		console.log(this.A_machineRoom_id);
		this.dialogFormVisible = true;
	...
	handleData (key, formName) {
		let detailData = [];
		for (var i=0; i<this.order_form.length; i++) {
			this.A_machineRoom_id = this.get_roomByid(this.order_form[i].A_machineBuilding, this.order_form[i].A_machineRoom);
			this.Z_machineRoom_id = this.get_roomByid(this.order_form[i].Z_machineBuilding, this.order_form[i].Z_machineRoom);
			detailData.push({
				bus_sys_name:this.order_form[i].bus_sys_name,
				...
				fk_rid_A:this.A_machineRoom_id,
				...
				fk_rid_Z:this.Z_machineRoom_id,
				...
				// notes: this.order_form_empty.notes
			})
		};
		...
		this.$refs[formName].validate((valid) => {
			if (valid) {
				...
				if (key == 1) {
					...
					json["application_detail"] = JSON.stringify(detailData);
				...
				if (key == 2) {
					...
					// 	})
					// }
					json["application_detail"] = JSON.stringify(that.order_form);
					var parmas = JSON.stringify(json);
					console.log(parmas);
					postTicketList(parmas).then((response)=> {
						console.log(response);
