  ...
  methods: {
	...
	get_roomByid (room_id) {
		...
		getBuildingRoomList({params:paramsData}).then((response)=> {
			// console.log(response);
			if (response.data.code == 1){
				// room_id = response.data.data.value[0].id;
				let room_data = response.data.data.value;
				console.log(room_data[0].room);
				return room_data[0].room;
				// console.log(room_id);
			}
		});
		// return room_id;
	...
	submitDialogForm(formName) {
		...
		if (this.data_dialogForm == this.order_form_empty) {
			...
			// this.A_machineRoom_id = this.get_roomByid(this.order_form_empty.A_machineBuilding, this.order_form_empty.A_machineRoom);
