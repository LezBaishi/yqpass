	...
	getOrderDetail (id) {
		...
		getTicketDetail(id).then((response)=> {
			...
			if (response.data.code == 1) {
				...
				console.log(list.application_detail.bus_sys_name)
				// this.order_form = list.application_detail;
				let list_detail = JSON.stringify(list.application_detail);
				// for (var i=0; i<list_detail.length; i++) {
					console.log(list_detail[1])
				// 	list["field_num"] = list_detail[i].field_num;
				// 	list["bus_sys_name"] = list_detail[i].bus_sys_name;
				// };
				// console.log(list);
