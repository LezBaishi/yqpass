...
from backend.workflow.models import Transition, CustomField, Workflow
...
class TicketBaseService(BaseService):
    ...
    @classmethod
    @auto_log
    def get_ticket_list(cls, sn='', title='', username='', workflow_id='',
                        create_start='', create_end='', category='', reverse=1,
                        per_page=10,
                        page=1):
        """
        获取工单列表
        ...
        if category == 'owner':
            ...
            if not user_obj:
                return False, msg
            user_dept_id_list, msg2 = AccountBaseService.get_user_dept_id_list(username)
            ...
            ticket_query_set1 = TicketRecord.objects.filter(query_params).extra(
                where=['FIND_IN_SET("{}", participant)'.format(username),
                       'participant_type_id={}'.format(CONSTANT_SERVICE.PARTICIPANT_TYPE_MULTI)])
        ...
        # 关联工单, 因为 relation 字段已经保留了工单的处理人信息, 所以只需要找到哪些工单这个字段中包含该 user 就ok了
            ...
            ticket_objects = TicketRecord.objects.filter(query_params).extra(
                where=['FIND_IN_SET("{}", relation)'.format(username)]).order_by(order_by_str)
        ...
        for ticket_result_object in ticket_result_object_list:
            # 工单状态与处理人信息
            state_obj, msg = WorkflowStateService.get_restful_state_info_by_id(ticket_result_object.state_id)
            ...
            ticket_result_restful_list.append(dict(id=ticket_result_object.id,
                                                   ...
                                                   state=dict(state_id=ticket_result_object.state_id,
    ...
    def new_ticket(cls, request_data_dict):
        ...
        # 提取工单字段列表
        request_field_arg_list = [key for key, value in request_data_dict.items() if
                                  (key not in ['workflow_id', 'suggestion', 'username'])]
        ...
        for key, value in state_field_dict.items():
            if value == str(CONSTANT_SERVICE.FIELD_ATTRIBUTE_REQUIRED):
                require_field_list.append(key)
                update_field_list.append(key)
            if value == str(CONSTANT_SERVICE.FIELD_ATTRIBUTE_OPTIONAL):
                update_field_list.append(key)
        ...
        if req_transition_obj.field_require_check:
            for require_field in require_field_list:
                if require_field not in request_field_arg_list:
                    return False, "new_ticket: 此工单必填字段为: {}".format(','.join(require_field_list))
        ...
        if not transition_queryset:
            return False, msg
        allow_transition_id_list = [str(transition.id) for transition in transition_queryset]

        if transition_id not in allow_transition_id_list:
            return False, "transition_id 不合法, 请提供正确的 transition_id"

        destination_state_obj = Transition.objects.filter(id=transition_id).first()
        ...
        if destination_participant_type_id == CONSTANT_SERVICE.PARTICIPANT_TYPE_FIELD:
            ...
            if not field_value:
                return False, "请求的数据中无此字段的值, 或值为空: {}".format(destination_participant)
        ...
        # 新建工单基础表数据
        new_ticket_obj = TicketRecord(sn=ticket_sn,
        ...
        # 新增流转记录
        new_ticket_flow_log_dict = dict(ticket_id=new_ticket_obj.id,
        ...
        if destination_state.type_id == CONSTANT_SERVICE.STATE_TYPE_END and new_ticket_obj.parent_ticket_id and \
            ...
            other_sub_ticket_queryset = TicketRecord.objects.filter(
                ...
                is_deleted=0).all()
            other_sub_ticket_state_id_list = [other_sub_ticket.state_id for other_sub_ticket in
                                              other_sub_ticket_queryset]
            if set(other_sub_ticket_state_id_list) == {new_ticket_obj.state_id}:
                ...
                cls.handle_ticket(parent_ticket_obj.id, dict(transition_id=parent_ticket_transition_id,
                                                             username='admin', suggestion='所有子工单处理完毕，自动流转'))
    ...
    def handle_ticket(cls, ticket_id, request_data_dict):
        ...
        # 校验当前工单必填字段
        state_obj, msg = WorkflowStateService.get_workflow_state_by_id(source_ticket_state_id)
        if not state_obj:
            return False, msg
        ...
        if req_transition_obj.field_require_check:
            ...
            for require_field in require_field_list:
                if require_field not in request_field_arg_list:
                    return False, "handle_ticket: 此工单的必填字段为: {}".format(','.join(require_field_list))

        # 获取 transition_id 对应的下个状态的信息
        transition_queryset, msg = WorkflowTransitionService.get_state_transition_queryset(source_ticket_state_id)
        ...
        if destination_participant_type_id == CONSTANT_SERVICE.PARTICIPANT_TYPE_DEPT:
            ...
            destination_participant, msg = cls.get_ticket_field_value(ticket_id, destination_participant)
            ...
            destination_participant, msg = cls.get_ticket_field_value(ticket_obj.parent_ticket_id,
        ...
        # 更新工单流转记录, 执行脚本, 通知信息等
        cls.add_ticket_flow_log(dict(ticket_id=ticket_id,
                                     transition_id=transition_id,
                                     suggestion=suggestion,
                                     participant_type_id=CONSTANT_SERVICE.PARTICIPANT_TYPE_PERSONAL,
                                     participant=username,
        ...
        if destination_state.type_id == CONSTANT_SERVICE.STATE_TYPE_END and ticket_obj.parent_ticket_id and ticket_obj.parent_ticket_state_id:
            ...
            other_sub_ticket_queryset = TicketRecord.objects.filter(parent_ticket_id=ticket_obj.parent_ticket_id,
        ...
        if destination_participant_type_id == CONSTANT_SERVICE.PARTICIPANT_TYPE_ROBOT:
            from backend.workflow.tasks import run_flow_task  # 放在文件开头会存在循环引用
            run_flow_task.apply_async(args=(ticket_id, destination_participant, destination_state_id,),
                                      queue='yqpass')
    ...
    def get_ticket_detail(cls, ticket_id, username):
        ...
        return dict(id=ticket_obj.id, sn=ticket_obj.sn, title=ticket_obj.title, state_id=ticket_obj.state_id,
                    ...
                    creator=ticket_obj.creator, gmt_created=str(ticket_obj.gmt_created)[0:19],
    ...
    def get_ticket_format_participant_info(cls, ticket_id):
        ...
        if participant_type_id == CONSTANT_SERVICE.PARTICIPANT_TYPE_PERSONAL:
            ...
            for name in participant_name_list:
                participant_user_obj, msg = AccountBaseService.get_user_by_username(name)
            ...
            participant_alias = participant_name
        return dict(participant=participant, participant_name=participant_name,
    ...
    def get_ticket_custom_field_value(cls, ticket_id, field_key):
        ...
        :return:
        """
        format_field_key_dict, msg = cls.get_ticket_custom_field_key_dict(ticket_id)
    ...
    def gen_ticket_sn(cls, workflow_id):
        ...
        tomorrow = str(datetime.datetime.now() + datetime.timedelta(days=1))[:10] + ' 00:00:00'
        print(today)
        print(tomorrow)
        ticket_day_count = TicketRecord.objects.filter(workflow_id=workflow_id,
                                                       gmt_created__gte=today,
                                                       gmt_created__lte=tomorrow).count()
        index_count = ticket_day_count + 1
        now_day = datetime.datetime.now()

        return 'YQ-{0:0>2}{1:0>4}{2:0>2}{3:0>2}-{4:0>3}'.format(workflow_id,
                                                                now_day.year,
                                                                now_day.month,
                                                                now_day.day,
                                                                index_count), ''
                ...
                if ticket_custom_field_queryset:
                    ...
                    if field_type_id == CONSTANT_SERVICE.FIELD_TYPE_STR:
                        new_ticket_custom_field_record = TicketCustomField(
                            name=format_custom_field_dict[key]['field_name'], ticket_id=ticket_id, field_key=key,
                            field_type_id=field_type_id, char_value=update_dict.get(key))
                    elif field_type_id == CONSTANT_SERVICE.FIELD_TYPE_INT:
                        new_ticket_custom_field_record = TicketCustomField(
                            name=format_custom_field_dict[key]['field_name'], ticket_id=ticket_id, field_key=key,
                            field_type_id=field_type_id, int_value=update_dict.get(key))
                    elif field_type_id == CONSTANT_SERVICE.FIELD_TYPE_FLOAT:
                        new_ticket_custom_field_record = TicketCustomField(
                            name=format_custom_field_dict[key]['field_name'], ticket_id=ticket_id, field_key=key,
                            field_type_id=field_type_id, float_value=update_dict.get(key))
                    elif field_type_id == CONSTANT_SERVICE.FIELD_TYPE_BOOL:
                        new_ticket_custom_field_record = TicketCustomField(
                            name=format_custom_field_dict[key]['field_name'], ticket_id=ticket_id, field_key=key,
                            field_type_id=field_type_id, bool_value=update_dict.get(key))
                    elif field_type_id == CONSTANT_SERVICE.FIELD_TYPE_DATE:
                        new_ticket_custom_field_record = TicketCustomField(
                            name=format_custom_field_dict[key]['field_name'], ticket_id=ticket_id, field_key=key,
                            field_type_id=field_type_id, date_value=update_dict.get(key))
                    elif field_type_id == CONSTANT_SERVICE.FIELD_TYPE_DATETIME:
                        new_ticket_custom_field_record = TicketCustomField(
                            name=format_custom_field_dict[key]['field_name'], ticket_id=ticket_id, field_key=key,
                            field_type_id=field_type_id, datetime_value=update_dict.get(key))
                    elif field_type_id == CONSTANT_SERVICE.FIELD_TYPE_RADIO:
                        new_ticket_custom_field_record = TicketCustomField(
                            name=format_custom_field_dict[key]['field_name'], ticket_id=ticket_id, field_key=key,
                            field_type_id=field_type_id, radio_value=update_dict.get(key))
                    elif field_type_id == CONSTANT_SERVICE.FIELD_TYPE_CHECKBOX:
                        new_ticket_custom_field_record = TicketCustomField(
                            name=format_custom_field_dict[key]['field_name'], ticket_id=ticket_id, field_key=key,
                            field_type_id=field_type_id, checkbox_value=update_dict.get(key))
                    elif field_type_id == CONSTANT_SERVICE.FIELD_TYPE_SELECT:
                        new_ticket_custom_field_record = TicketCustomField(
                            name=format_custom_field_dict[key]['field_name'], ticket_id=ticket_id, field_key=key,
                            field_type_id=field_type_id, select_value=update_dict.get(key))
                    elif field_type_id == CONSTANT_SERVICE.FIELD_TYPE_MULTI_SELECT:
                        new_ticket_custom_field_record = TicketCustomField(
                            name=format_custom_field_dict[key]['field_name'], ticket_id=ticket_id, field_key=key,
                            field_type_id=field_type_id, multi_select_value=update_dict.get(key))
                    elif field_type_id == CONSTANT_SERVICE.FIELD_TYPE_TEXT:
                        new_ticket_custom_field_record = TicketCustomField(
                            name=format_custom_field_dict[key]['field_name'], ticket_id=ticket_id, field_key=key,
                            field_type_id=field_type_id, text_value=update_dict.get(key))
                    elif field_type_id == CONSTANT_SERVICE.FIELD_TYPE_USERNAME:
                        new_ticket_custom_field_record = TicketCustomField(
                            name=format_custom_field_dict[key]['field_name'], ticket_id=ticket_id, field_key=key,
                            field_type_id=field_type_id, username_value=update_dict.get(key))
                    elif field_type_id == CONSTANT_SERVICE.FIELD_TYPE_MULTI_USERNAME:
                        new_ticket_custom_field_record = TicketCustomField(
                            name=format_custom_field_dict[key]['field_name'], ticket_id=ticket_id, field_key=key,
                            field_type_id=field_type_id, multi_username_value=update_dict.get(key))
    ...
    def ticket_handle_permission_check(cls, ticket_id, username):
        ...
        if participant_type_id == CONSTANT_SERVICE.PARTICIPANT_TYPE_PERSONAL:
            ...
            if username not in participant.split(',') and not user_obj.is_staff:
                return False, "非当前处理人, 无权处理"
            ...
            role_user_list, msg = AccountBaseService.get_role_username_list(role_id=int(participant))
    ...
    def add_ticket_relation(cls, ticket_id, user_str):
        ...
        # 去重 and 去空元素
        new_relation_set = set(ticket_obj.relation.split(',') + user_str.split(','))
        new_relation_list = [relation for relation in new_relation_set if relation]
    ...
    def get_ticket_base_field_list(cls, ticket_id):
        ...
        field_list.append(dict(field_key='sn', name="工单编号", value=ticket_obj.sn, order_id=0,
                               field_type_id=CONSTANT_SERVICE.FIELD_TYPE_STR,
                               field_attribute=CONSTANT_SERVICE.FIELD_ATTRIBUTE_RO))
        field_list.append(dict(field_key='title', name="标题", value=ticket_obj.title, order_id=20,
                               field_type_id=CONSTANT_SERVICE.FIELD_TYPE_STR,
                               field_attribute=CONSTANT_SERVICE.FIELD_ATTRIBUTE_RO))
        field_list.append(dict(field_key='state_id', name="状态", value=ticket_obj.state_id, order_id=40,
                               field_type_id=CONSTANT_SERVICE.FIELD_TYPE_STR,
                               field_attribute=CONSTANT_SERVICE.FIELD_ATTRIBUTE_RO))
        field_list.append(dict(field_key='participant_info.participant_name', name="当前处理人",
                               value=participant_info_dict['participant_name'], order_id=50,
                               field_type_id=CONSTANT_SERVICE.FIELD_TYPE_STR,
                               field_attribute=CONSTANT_SERVICE.FIELD_ATTRIBUTE_RO))
        field_list.append(dict(field_key='participant_info.participant_alias', name="当前处理人",
                               value=participant_info_dict['participant_alias'], order_id=55,
                               field_type_id=CONSTANT_SERVICE.FIELD_TYPE_STR,
                               field_attribute=CONSTANT_SERVICE.FIELD_ATTRIBUTE_RO))
        field_list.append(dict(field_key='workflow.workflow_name', name="工作流名称",
                               value=workflow_name, order_id=60,
                               field_type_id=CONSTANT_SERVICE.FIELD_TYPE_STR,
                               field_attribute=CONSTANT_SERVICE.FIELD_ATTRIBUTE_RO))
        field_list.append(dict(field_key='creator', name="创建人",
                               value=ticket_obj.creator, order_id=80,
                               field_type_id=CONSTANT_SERVICE.FIELD_TYPE_STR,
                               field_attribute=CONSTANT_SERVICE.FIELD_ATTRIBUTE_RO))
        field_list.append(dict(field_key='gmt_created', name="创建时间",
                               value=str(ticket_obj.gmt_created)[:19], order_id=100,
                               field_type_id=CONSTANT_SERVICE.FIELD_TYPE_STR,
                               field_attribute=CONSTANT_SERVICE.FIELD_ATTRIBUTE_RO))
        field_list.append(dict(field_key='gmt_modified', name="更新时间",
                               value=str(ticket_obj.gmt_modified)[:19], order_id=120,
                               field_type_id=CONSTANT_SERVICE.FIELD_TYPE_STR,
                               field_attribute=CONSTANT_SERVICE.FIELD_ATTRIBUTE_RO))
        field_list.append(dict(field_key='state.state_name', name="状态名",
                               value=state_name, order_id=41,
                               field_type_id=CONSTANT_SERVICE.FIELD_TYPE_STR,
                               field_attribute=CONSTANT_SERVICE.FIELD_ATTRIBUTE_RO))
        ...
        for key, value in custom_field_dict.items():
            ...
            ticket_custom_field_obj = TicketCustomField.objects.filter(ticket_id=ticket_id, field_key=key,
                                                                       is_deleted=0).first()
            # 尚未赋值
            if not ticket_custom_field_obj:
                field_value = None
            ...
            field_list.append(
                dict(field_key=key, field_name=custom_field_dict[key]['field_name'], field_value=field_value,
    ...
    def get_ticket_transition(cls, ticket_id, username):
        ...
        if ticket_obj.in_add_node:
            ...
            transition_dict_list = [
                dict(transition_id=0, transition_name='完成', field_require_check=False, is_accept=False,
                     in_add_node=True)]
            return transition_dict_list, ''
        if msg['need_accept']:
            transition_dict_list = [
                dict(transition_id=0, transition_name='接单', field_require_check=False, is_accept=True,
                     in_add_node=False)]
            return transition_dict_list, ''
    ...
    @classmethod
    @auto_log
    def get_ticket_flow_log(cls, ticket_id, username, per_page, page):
        """
        获取工单流转记录
        :param ticket_id:
        :param username:
        ...
        for ticket_flow_log in ticket_result_paginator.object_list:
            ...
            ticket_flow_log_restful_list.append(dict(
                id=ticket_flow_log.id, ticket_id=ticket_id, state=state_info_dict, transition=transition_info_dict,
                intervene_type_id=ticket_flow_log.intervene_type_id, participant=ticket_flow_log.participant,
                participant_type_id=ticket_flow_log.participant_type_id, suggestion=ticket_flow_log.suggestion,
                    ...
                    if ticket_flow_log.state_id == state_obj.id:
                        if ticket_flow_log.transition_id:
                            ...
                            if ticket_flow_log.intervene_type_id == CONSTANT_SERVICE.TRANSITION_INTERVENE_TYPE_DELIVER:
                                ...
                                transition_name = '未知操作'
                        state_flow_log_list.append(dict(id=ticket_flow_log.id,
                                                        transition=dict(transition_name=transition_name,
    ...
    def accept_ticket(cls, ticket_id, username):
        ...
        if msg['need_accept']:
            ...
            ticket_flow_log_dict = dict(ticket_id=ticket_id, transition=0, suggestion='接单处理',
                                        participant_type_id=CONSTANT_SERVICE.PARTICIPANT_TYPE_PERSONAL,
                                        participant=username,
                                        intervene_type_id=CONSTANT_SERVICE.TRANSITION_INTERVENE_TYPE_ACCEPT,
                                        state_id=ticket_obj.state_id, creator=username)
    ...
    def deliver_ticket(cls, ticket_id, username, target_username, suggestion):
        ...
        if not permission:
            return False, msg
        user_obj, msg = AccountBaseService.get_user_by_username(target_username)
        if not user_obj:
    ...
    def ticket_delete(cls, username, delete_str):
        ...
        permission = cls.ticket_delete_permission_check(username)
        if not permission:
            return False, "用户权限不够"

        deleted_list = []
        ticket_objs = TicketRecord.objects.filter(pk__in=delete_list, is_deleted=False)
        if not ticket_objs:
    ...
    @classmethod
    @auto_log
    def ticket_delete_permission_check(cls, username):
        """
        校验管理员权限
        :param username:
        :return:
